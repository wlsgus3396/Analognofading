...
class CSVec(object):
    ...
    def _findHHK(self, k):
        ...
        """HHs= heavyhitter의 index 값 .long()= 부호있는 64비트 정수형"""
        torch.topk(vals**2, k, sorted=False, out=(outVals, HHs))
        """ vals값을 제곱하여 요소값이 가장 큰 순서대로 k만큼 추출한다.
            sorted=True 이면, 소티드 된 순서대로 값은 반환한다.
            outVals는 제곱된 값이므로 쓰지 않는다."""
        return HHs, vals[HHs]
        """ 인덱스값과, vals에서의 해당 밸류를 리턴한다."""
    ...
    """해석 완료"""
    """ 해쉬값을 역적용하고, median을 통해서 모든 값을 복구시킨다. """
        ...
        if self.numBlocks == 1:
            ...
            for r in range(self.r):
                vals[r] = (self.table[r, self.buckets[r,:]]
                           * self.signs[r,:])
            return vals.median(dim=0)[0]
            # vals.median(dim=0) 각 열에 대한 중위값을 구한다.
            # vals.median(dim=0)[0] = 중위값
            # vals.median(dim=0)[1] = 중위값에 대한 인덱스
            # 결국 d에 부여되었던 모든 값을 복구 시킨다.
    ...
    def unSketch(self, k=None, epsilon=None):
        ...
        if epsilon is None:  # 엡실론 값이 없으면, 트레숄드값도 none이고, 이것은 hhk를 선택하게한다.
            thr = None
        else:
            thr = epsilon * self.l2estimate() # 엡실론 주어지면, 헤비히터 구하기 위한 트레숄드값을 구한다.

        hhs = self._findHHs(k=k, thr=thr)  # hhs는 두가지 방식중 하나를 선택하여, 헤비히터값을 반환받는다.

        if k is not None:
            assert(len(hhs[1]) == k) # k가 주어졌다면, 헤비히터 값의 개수는 k와 같은지 확인
        if epsilon is not None:
            assert((hhs[1] < thr).sum() == 0) # 엡실론이 주어졌다면, 헤비히터값중 thr 작은것들의합은 0이어야한다.
    ...
    def l2estimate(self):
        ...
        """torch.sum(self.table**2,1) = 각 요소를 제곱한뒤, 행 요소값을 다 더한다. 
          torch.median(A)= 중위값을 텐서로 반환한다. .item()을 하면 스칼라값으로 변환한다.
          l2_norm은 각 행별로 계산되므로, 그중에서 중위값을 취한다."""
