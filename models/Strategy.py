...
""""###########################################################################################"""
""""###########################################################################################"""
def FedSketch(w, previous_w_global,T,P,K,cc, g_ec,s_momentum,po,add_momentum,device,iter,column,add_error,lr):
...
########################################################################################################################
    ...
    """weight 업데이트"""
    Fw_avg = copy.deepcopy(D)
    Fw_avg -=lr * return_topk

    # Fw_avg = copy.deepcopy(D)
    # for k in range(0, d):
    #     Fw_avg[k] -= lr * return_topk[k]
...
""""###########################################################################################"""
""""###########################################################################################"""
def Fedtopk(w, previous_w_global, T, P, g_ec,po,device,add_error,lr):
    model = []
    for k in w[0].keys():
        model.append(w[0][k].shape)
    ...
    for i in range(0,len(w)):
        Gprime = torch.sort(torch.abs(OG[i]),descending=True)[0]
        # 2T번째 값 구하기
        topk=Gprime[2*T-1]

        # topk보다 작은 값은 0으로 만들기
        indexing=torch.abs(OG[i])<topk
        # 1,0으로 표현하기
    ...
    for i in range(0,len(w)):
        restored_gradient+=pattern_gradient[i]
    restored_gradient=restored_gradient/len(w)

    Fw_avg = copy.deepcopy(D)
    Fw_avg -= lr * restored_gradient################################################################################################################################
...
def FedAnalogRandom(w, previous_w_global, T, P, g_ec,po,device,add_error,lr):
    model = []
    for k in w[0].keys():
        model.append(w[0][k].shape)
    ...
    for m in range(len(w)):
        h2_matrix[m] = sampling.sample([1])#############################################################################
    gradient_sum_timeslot = {}
    for m in range(len(w)):
        gradient_sum_timeslot[m] = sum((compressed_gradient[m])**2)############################################################################################

    ####################################################################################################################
    # gamma 구하기
    gamma = []
    for m in range(len(w)):
        gamma.append(torch.sqrt(h2_matrix[m].to(device)*T*po / gradient_sum_timeslot[m]) )
    ...
    for m in range(len(w)):
        re_signal += gamma_avg * compressed_gradient[m] 
    ...
    re_signal_noise = re_signal_noise / len(w)
...
def FedAnalogPSGuide(w, previous_w_global, T, P, g_ec,po,device, K, add_error,lr):
...
""""###########################################################################################"""
""""###########################################################################################"""
def FedAnalogOneDeviceGuide(w, previous_w_global, T, P, g_ec,po,device, K, add_error,r,lr):
    model = []
    for k in w[0].keys():
        model.append(w[0][k].shape)
    ...
    for m in range(len(w)):
        h2_matrix[m] = sampling.sample([1,1])#############################################################################
    
    H=max(list(h2_matrix.values()))
    Index=list(h2_matrix.values()).index(H)
    ...
    p_allo_h2_matrix=po*(T/T1)*H+1
    capacity=T1*torch.log2(p_allo_h2_matrix)
    
    
    qt = 1
    while nCr(d, qt) < 2**(int(capacity)):
        qt += 1
        if qt>int(d/2):
            break
    ...
    if qt==0:
        ...
        SP = torch.zeros(d).to(device)
        SP[Sp]=1
        Sp=torch.nonzero(SP).reshape(2*T).to(device)
        compressed_gradient = torch.zeros(len(w), 2 * T).to(device)
    else:
        if 2*T2>qt:
            if 2*T2>d-qt:
                qt=d-qt
                Gprime=torch.sort(torch.abs(OG[Index]),descending=True)[0]
                topk=Gprime[qt-1]
                # topk보다 작은 값은 0으로 만들기
                indexing=torch.abs(OG[Index])<topk
                # 1,0으로 표현하기
                SP = torch.ones(d).to(device)
                SP[indexing]=0
    ...
    for i in range(0,len(w)):
        ...
        if i==Index:
            if qt!=0:
                pattern_gradient[i]*=0
                compressed_gradient[i]*=0
    ...
    if qt==0:
        ...
        re_signal = torch.zeros(1, 2 * T2).to(device)
        noise = ((torch.randn(1, 2 * T2).to(device)) / np.sqrt(2))
    for m in range(len(w)):
        re_signal += gamma_avg * compressed_gradient[m] 
    ...
    if qt==0:
        ...
        re_signal_noise = re_signal_noise / (len(w)-1)
    G_avg = re_signal_noise / gamma_avg
...
""""###########################################################################################"""
""""###########################################################################################"""
def FedAnalogAMP(w, previous_w_global,T,P,g_ec,po,add_error,lr):
    ...
    # A매트릭스와 곱하여서 압축하기  d개(d) -> 2T개(10000개)
    ...
    gradient_sum_timeslot = {}
    for m in range(len(w)):
        gradient_sum_timeslot[m] = sum((Gcomp[m])**2)############################################################################################

    ####################################################################################################################
    # gamma 구하기
    gamma = []
    for m in range(len(w)):
        gamma.append(torch.sqrt(h2_matrix[m]*T*po / gradient_sum_timeslot[m]) )
    ...
    """# Received signal 구하기: 식(22)"""
    re_signal = torch.zeros(1, 2 * T)

    for m in range(len(w)):
        re_signal += gamma_avg * Gcomp[m] 

    # 가우시안 노이즈 생성 및 노이즈 더하기
    noise = ((torch.randn(1, 2 * T)) / np.sqrt(2))
    re_signal_noise = re_signal+noise

    # 인원수로 나누고, 감마평균값으로 나누기
    re_signal_noise = re_signal_noise / len(w)
    Gcomp_avg = re_signal_noise / gamma_avg
    ...
    #     Fw_avg[k]-=lr*G_avg[k]
    #     # 모멘텀 주기
    #     # Fw_avg[k] = 0.9*Fw_avg[k] - lr*G_avg[k]
...
def Fed_DDSGD(w, previous_w_global,T,P,g_ec,po, device,lr):
    ...
    H=min(list(h2_matrix.values()))
    Index=list(h2_matrix.values()).index(H)


    p_allo_h2_matrix=po*H+1
    capacity=T*torch.log2(p_allo_h2_matrix)
    ...
    # 유저별 q_t 구하기
    qt_gradient = torch.zeros([len(w),d]).to(device)
    index_info=torch.zeros([d]).to(device)


    # qt 개수가지고 gradient 처리하기
    for i in range(len(w)):
        positive_qt, index_po = torch.topk(OG[i], int(qt), largest=True)
        negative_qt, index_ne = torch.topk(OG[i], int(qt), largest=False)
...
def sign_sgd(w, previous_w_global,T,P,g_ec,po, device,lr):
    ...
    qt = 1
    while nCr(d, qt) < 2**(int(capacity)-qt):
        qt += 1
    # 최종 q_t 개수: while문에서 마지막에 1 더해진 값 뺴기
    qt -= 1
    #채널 에러 방지
    ...
    for i in range(len(w)):
        q_t, index_qt = torch.topk(abs(OG[i]), int(qt), largest=True)

        sign_G = torch.zeros(int(qt)).to(device)

        # 0보다 크면 1, 0보다 작으면 -1로 설정하기
        for j in range(int(qt)):
...
""""###########################################################################################"""
""""###########################################################################################"""
def sign_major(w, previous_w_global,T,P,g_ec,po, device,lr):
    ...
    majority_qt=torch.zeros([d]).to(device)
