...
def FedSketch(w, previous_w_global, T, P, K, cc, g_ec, s_momentum, po, add_momentum):
    ...
    # 각 유저별 스케칭 준비
    Sketch_0 = CSVec(dd, cc, rr, device="cpu")
    Sketch_1 = CSVec(dd, cc, rr, device="cpu")
    Sketch_2 = CSVec(dd, cc, rr, device="cpu")
    Sketch_3 = CSVec(dd, cc, rr, device="cpu")
    Sketch_4 = CSVec(dd, cc, rr, device="cpu")
    Sketch_5 = CSVec(dd, cc, rr, device="cpu")
    Sketch_6 = CSVec(dd, cc, rr, device="cpu")
    Sketch_7 = CSVec(dd, cc, rr, device="cpu")
    Sketch_8 = CSVec(dd, cc, rr, device="cpu")
    Sketch_9 = CSVec(dd, cc, rr, device="cpu")

    # 인원수 늘리기

    Sketch_10 = CSVec(dd, cc, rr, device="cpu")
    Sketch_11 = CSVec(dd, cc, rr, device="cpu")
    Sketch_12 = CSVec(dd, cc, rr, device="cpu")
    Sketch_13 = CSVec(dd, cc, rr, device="cpu")
    Sketch_14 = CSVec(dd, cc, rr, device="cpu")
    Sketch_15 = CSVec(dd, cc, rr, device="cpu")
    Sketch_16 = CSVec(dd, cc, rr, device="cpu")
    Sketch_17 = CSVec(dd, cc, rr, device="cpu")
    Sketch_18 = CSVec(dd, cc, rr, device="cpu")
    Sketch_19 = CSVec(dd, cc, rr, device="cpu")
    Sketch_20 = CSVec(dd, cc, rr, device="cpu")
    Sketch_21 = CSVec(dd, cc, rr, device="cpu")
    Sketch_22 = CSVec(dd, cc, rr, device="cpu")
    Sketch_23 = CSVec(dd, cc, rr, device="cpu")
    Sketch_24 = CSVec(dd, cc, rr, device="cpu")
    Sketch_25 = CSVec(dd, cc, rr, device="cpu")
    Sketch_26 = CSVec(dd, cc, rr, device="cpu")
    Sketch_27 = CSVec(dd, cc, rr, device="cpu")
    Sketch_28 = CSVec(dd, cc, rr, device="cpu")
    Sketch_29 = CSVec(dd, cc, rr, device="cpu")
    Sketch_30 = CSVec(dd, cc, rr, device="cpu")
    Sketch_31 = CSVec(dd, cc, rr, device="cpu")
    Sketch_32 = CSVec(dd, cc, rr, device="cpu")
    Sketch_33 = CSVec(dd, cc, rr, device="cpu")
    Sketch_34 = CSVec(dd, cc, rr, device="cpu")
    Sketch_35 = CSVec(dd, cc, rr, device="cpu")
    Sketch_36 = CSVec(dd, cc, rr, device="cpu")
    Sketch_37 = CSVec(dd, cc, rr, device="cpu")
    Sketch_38 = CSVec(dd, cc, rr, device="cpu")
    Sketch_39 = CSVec(dd, cc, rr, device="cpu")
    Sketch_40 = CSVec(dd, cc, rr, device="cpu")
    Sketch_41 = CSVec(dd, cc, rr, device="cpu")
    Sketch_42 = CSVec(dd, cc, rr, device="cpu")
    Sketch_43 = CSVec(dd, cc, rr, device="cpu")
    Sketch_44 = CSVec(dd, cc, rr, device="cpu")
    Sketch_45 = CSVec(dd, cc, rr, device="cpu")
    Sketch_46 = CSVec(dd, cc, rr, device="cpu")
    Sketch_47 = CSVec(dd, cc, rr, device="cpu")
    Sketch_48 = CSVec(dd, cc, rr, device="cpu")
    Sketch_49 = CSVec(dd, cc, rr, device="cpu")
    ...
    sum_sign_matrix = torch.zeros(1, 2 * T)
    for m in range(len(w)):
    ...
    for m in range(len(w)):
        for i in range(int(T / 50)):  # Number of timeslot
            gamma.append((np.sqrt(sampling.sample([1]))) * (
                        np.sqrt(po / gradient_sum_timeslot[m][i]) / np.sqrt(6.33153936413615)))
    # gamma list -> gamma matrix로 변형
    gamma_matrix = torch.tensor(gamma).reshape(len(w), int(T / 50))
    ...
    gamma_vector_cat = {}
    for m in range(len(w)):
        gamma_vector_T = torch.zeros([int(T / 50), 50])
    ...
    received_signal = torch.zeros(1, 2 * T)
    for m in range(len(w)):
        received_signal += gamma_vector_cat[m] * sketch_to_vector[m] * sign_matrix[m]
    # 가우시안 노이즈 더하기
    received_signal += ((torch.randn(1, 2 * T)) / np.sqrt(2))
...
def Fed_DDSGD(w, previous_w_global, T, P, g_ec, po, device):
    ...
    h2_matrix = m.sample([len(w), T])

    # argmax{sum(h^2)}
    h2_matrix_sum = h2_matrix.sum(axis=1)
    values, index = torch.topk(h2_matrix_sum, 1)  # index에 가장 큰값을 가진 유저번호가 저장되어있음.

    # 선택된 유저에게 water filling 적용
    noise_vec = 1 / h2_matrix[index[0]]
    p_constrain = po * len(w)
    tol = 1e-2  # 1e-5 에서 1e-4로 수정
    number_vector = len(noise_vec)

    water_line = min(noise_vec) + p_constrain / number_vector  # 초기 워터라인 설정
    p_allocation = water_line - noise_vec  # 초기 파워 할당

    for i in range(number_vector):  # 파워 값 마니너스 일 경우 0으로 설정
        if p_allocation[i] < 0:
            p_allocation[i] = 0
    ...
    # 선택된 유저와, 중앙서버 간의 capacity 구하기
    p_allo_h2_matrix = p_allocation * h2_matrix[index[0]]
    p_allo_h2_matrix += 1
    capacitylog = torch.log2(p_allo_h2_matrix)
    capacity = sum(capacitylog)
    ...
    for k in w[0].keys():
        W.append(previous_w_global[k])
    G_D, G_L = flatten_params(W)

    # Gradient 구하기
    new_G = torch.zeros([21840])
    new_G.to(device)
    for k in range(0, d):
        new_G[k] = (G_D[k] - D[k]) / 0.01 + g_ec[index[0]][k]

    """############"""
    """q_t 구하기"""
    # Upperbound 구하기

    upper = 2 ** (int(capacity) - 33)
    ...
    # upper 만족하는 number_qt 구하기
    number_qt = 1
    while nCr(d, number_qt) < upper:
        number_qt += 1
    ...
    positive_qt, index_po = torch.topk(new_G, number_qt, largest=True)
    negative_qt, index_ne = torch.topk(new_G, number_qt, largest=False)

    positive_mean = torch.mean(positive_qt)
    negative_mean = abs(torch.mean(negative_qt))

    # q_t_gradient=torch.zeros([21840],device="cuda:0")
    q_t_gradient = torch.zeros([21840])
    q_t_gradient.to(device)
    # q_t_gradient = torch.zeros(21840)
    if positive_mean > negative_mean:
        q_t_gradient[index_po] = positive_mean
    else:
        q_t_gradient[index_ne] = torch.mean(negative_qt)

    # error 누적하기
    g_ec[index[0]] = new_G - q_t_gradient

    """weight 업데이트"""
    Fw_avg = copy.deepcopy(D)
    for k in range(0, d):
        Fw_avg[k] -= 0.01 * q_t_gradient[k]
...
def sign_sgd(w, previous_w_global, T, P, g_ec, po, device):
    ...
    while ((nCr(d, number_qt)) - (
            2 ** (int(capacity) - number_qt))) < 0:  # capacity를 int형으로 변환해도 되는지? infinity 막기 위해서: 결론 별차이 없다.
        number_qt += 1

    # while (torch.log2(torch.tensor(float(nCr(d,number_qt)))))-float(capacity)+number_qt<0:
    #     number_qt+=1
    # ipdb.set_trace()
    # while (np.log2(np.array(nCr(d,number_qt))))-float(capacity)+number_qt<0:
    #     number_qt += 1
    # ipdb.set_trace()
    ...
    # TOPK 값 받기
    q_t, index_qt = torch.topk(abs(new_G), number_qt, largest=True)

    sign_G = torch.zeros([number_qt])
    sign_G.to(device)

    # 0보다 크면 1, 0보다 작으면 -1로 설정하기
    for i in range(number_qt):
        if new_G[index_qt][i] > 0:
            sign_G[i] = 1
        else:
            sign_G[i] = -1
